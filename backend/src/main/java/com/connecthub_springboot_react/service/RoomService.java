...
public class RoomService {
    ...
    public void deleteRoom(String hostUserId, String roomId) {
        Room room = roomRepository.findById(roomId).orElseThrow(() -> new IllegalArgumentException("Room not found"));
        if (!room.getHostUserId().equals(hostUserId)) {
            throw new IllegalStateException("Only the host can terminate the room");
        }
        stageRequestRepository.deleteByRoomId(roomId);
